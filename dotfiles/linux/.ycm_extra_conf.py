...
compilation_database_folder = '/home/' + os.getenv('USER') + '/fbcode/_build/dev/'
    ...
    if database:
      ...
      for f in os.listdir(directory):
        ...
        if f.endswith(".cpp"):
          ...
          print filename
          info = database.GetCompilationInfoForFile(
              filename)
...
def FlagsForFile( filename, **kwargs ):
  if database:
    ...
    if not compilation_info or len(compilation_info.compiler_flags_) == 0:
      ...
      if not compilation_info or len(compilation_info.compiler_flags_) == 0:
        compilation_info = GetAlternativeFileAtPeerDirectories( filename )
        if not compilation_info or len(compilation_info.compiler_flags_) == 0:
          return None
    ...
    final_flags = MakeRelativePathsInFlagsAbsolute(flags_fbcode, relative_to)

  final_flags.extend([ '-isystem','/usr/lib',
                       '-isystem','/usr/include',
