...
class ConsoleDiff(object):
    ...
    def _display_len(self, s):
        ...
        def width(c):
            if isinstance(c, type(u"")) and unicodedata.east_asian_width(c) == "W":
                return 2
            elif c == "\r":
                return 2
            return 1
    ...
    def _collect_lines(self, diffs):
        ...
        for fromdata, todata, flag in diffs:
            if (fromdata, todata, flag) == (None, None, None):
                ...
                yield (self._format_line(*fromdata), self._format_line(*todata))
    ...
    def colorize(self, s):
        ...
        if not self.show_all_spaces:
            ...
            # don't color it.
            return re.sub(
                "\033\\[[01];3([01234567])m(\\s+)(\033\\[)", "\033[7;3\\1m\\2\\3", s
            )
...
def create_option_parser():
    ...
    parser.add_option(
        "--head", default=0, help="consider only the first N lines of each file"
    )
    parser.add_option(
        "-H",
        "--highlight",
    ...
    parser.add_option(
        ...
        default=False,
        action="store_true",
        help="strip any trailing carriage return at the end of " "an input line",
    )
    parser.add_option(
        "--color-map",
        default=None,
...
def diff(options, a, b):
    ...
    is_a_file = a is not None and not os.path.isdir(a)
    is_b_file = b is not None and not os.path.isdir(b)

    if (is_a_file or a is None) and (is_b_file or b is None):
        assert a is not None or b is not None
        if a is None:
            print_meta("added %s" % b)
        elif b is None:
            print_meta("deleted %s" % a)
        try:
        ...
        a_contents = set(os.listdir(a)) if a is not None else set()
        b_contents = set(os.listdir(b)) if b is not None else set()

        for child in sorted(a_contents.union(b_contents)):
            a_child = os.path.join(a, child) if a is not None else None
            b_child = os.path.join(b, child) if b is not None else None
            a_child = None if child not in a_contents else a_child
            b_child = None if child not in b_contents else b_child
            if options.recursive:
                diff(options, a_child, b_child)
...
def read_file(fname, options):
    try:
        ...
        codec_print("error: encoding '%s' was not found." % (options.encoding), options)
        sys.exit()
...
def diff_files(options, a, b):
    if options.is_git_diff is True:
        ...
        if options.labels:
            if len(options.labels) == 2:
                ...
                codec_print(
                    "error: to use arbitrary file labels, " "specify -L twice.", options
                )
                return
        else:
    ...
    assert a is None or not os.path.isdir(a), a
    assert b is None or not os.path.isdir(b), b

    try:
        lines_a = read_file(a, options) if a is not None else []
        lines_b = read_file(b, options) if b is not None else []
    ...
    if options.matcher:
        lines_a = [
            line_a for line_a in lines_a if not re.search(options.matcher, line_a)
        ]
        lines_b = [
            line_b for line_b in lines_b if not re.search(options.matcher, line_b)
        ]
            ...
            if color not in color_codes:
                print(
                    ...
                    % (
                        ...
                        command_for_errors,
                        ", ".join(
                            [raw_colorize(x, x) for x in sorted(color_codes.keys())]
                        ),
                    )
                )
                sys.exit()
